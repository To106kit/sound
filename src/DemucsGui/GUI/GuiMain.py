...
t_outputBaseDir = "D:\\11_github\\sound\\result\\demcsgui"
t_exist_dir = os.listdir(t_outputBaseDir)
t_sufix_str = "_separated"
os.chdir(homeDir)
...
def LoadModel():
    ...
    t_path_list = glob.glob('D:\\11_github\\sound\\result\\dlyoutubemp3\\**\\*mp3')

    for t_path in t_path_list:
        t_filename = os.path.basename(t_path)
        t_filename = t_filename + t_sufix_str
        # print(t_filename)
        # print(t_exist_dir)
...
def Separate(File: pathlib.Path):
    ...
    try:
        ...
        RWCore.process(
            ...
            File,
            True,
            pathlib.Path(t_outputBaseDir + "/" + File.name + "_separated"),
            int(PPE.get()),
            float(POE.get()),
            model.samplerate,
            int(PHE.get()),
...
# def maintest():
    ...
    # MPV = tkinter.StringVar(MLF, "d:\\11_github\\python\\sound\\venv\\demucs_venv\\Demucs-Gui\\GUI\\pretrained")
    # MPV = tkinter.StringVar(MLF, "unch")
    ...
    Start()
    _thread.start_new_thread(LoadModel, ())
    print("test")    
...
# 必要な入力
## 1ページ目
# - model name: mdx_extra_q
# - model path: d:\11_github\python\sound\venv\demucs_venv\Demucs-Gui\GUI\pretrained
# - load model ボタン: 押下

## 2ページ目
# - Device: CUDA:0 - GeForce RTX 2060 SUPER (8192 MiB)
# - split: 44
# - overlap: 0.25
# - browse file to separate(.mp3)
